...
TEST_F(LightFieldTestFixture, testPanoramicConstructor) {
    PanoramicVideoLightField<EquirectangularGeometry, YUVColorSpace>({0, 10},
                                                                     std::ifstream{"resources/test-pattern.h264"});
}

TEST_F(LightFieldTestFixture, testPanoramicValueOutOfVolume) {
    auto field = PanoramicVideoLightField<EquirectangularGeometry, YUVColorSpace>({0, 10},
                                                                                  std::ifstream{"resources/test-pattern.h264"});
