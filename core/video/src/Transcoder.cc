...
Transcoder2::Transcoder2(CUcontext context, EncodeConfig& configuration)
        : context(context), lock(context), encoder(lock.get()), frameQueue(lock.get()), configuration(configuration), fpsRatio(1) {
...
void Transcoder2::InitializeDecoder(const std::string &inputFilename) {
    NVENCSTATUS nvStatus;
...
int Transcoder2::InitializeEncoder(const std::string &outputFilename) {
    NVENCSTATUS nvStatus;
...
int Transcoder2::transcode(const std::string &inputFilename, const std::string &outputFilename) {
    int result;
    NVENCSTATUS nvStatus;
...
Transcoder::Transcoder(unsigned int height, unsigned int width, unsigned int codec, std::string preset,
  ...
  // const_cast<char*>(this->inputFilename.c_str()); //TODO
  // configuration.outputFileName =
  // const_cast<char*>(this->outputFilename.c_str()); //TODO
}
...
int Transcoder::initialize() {
  ...
  /*
          ...
          decoder.InitVideoDecoder(configuration.inputFileName, lock,
     frameQueue, configuration.width, configuration.height);
     ...
     &isProgressive);
          ...
          if (configuration.fps <= 0) {
                  ...
                  fpsRatio = static_cast<float>(configuration.fps) * decodedFRD
     / decodedFRN;

          configuration.pictureStruct = (isProgressive ? NV_ENC_PIC_STRUCT_FRAME
     : 0);
          frameQueue->init(configuration.width, configuration.height);
  */
  fpsRatio = 1.f;
