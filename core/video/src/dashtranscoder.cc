...
namespace Python
{
	...
	{
	public:
		Tiler(unsigned int height, unsigned int width,
			...
			configurations(getConfigurations(
				inputFilename,
				height, width,
				tileRows, tileColumns, // TODO change parameter to TileDimensions&
				qualities,
				codec, fps,
				gop_length, deviceId)), 
		...
		static std::vector<EncodeConfig> getConfigurations(
			char *inputFilename,
			...
			unsigned int gop_length, unsigned int deviceId)
		{
			...
			for (auto i = 0; i < length; i++) {
				...
				configurations.push_back(MakeTilerConfiguration(
					inputFilename, outputFilenameFormat,
					height, width,
					tileRows, tileColumns,
					codec, preset, fps, gop_length,
					bitrate, rate_control_mode, deviceId));
			}
		...
		bool complete()
		{
			return tileFuture.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready;
		}
	...
	{
		boost::python::class_<Tiler>("Tiler", boost::python::init<
		...
		boost::python::class_<Transcoder>("Transcoder", boost::python::init<
			unsigned int, unsigned int,
