...
void EncodeWorker(EncodeContext& context)
	//CudaDecoder& decoder, TileVideoEncoder& encoder, CUVIDFrameQueue& queue, EncodeConfig& configuration,
      //            float fpsRatio)
{
    auto frmProcessed = 0;
    auto frmActual = 0;
...
int InitializeEncoder(CUcontext& cudaContext, EncodeConfig& configuration, unsigned long bitrate, int encoderIndex, TileVideoEncoder& encoder, const TileDimensions& tileDimensions)
{
	NVENCSTATUS status;

	//    configuration.presetGUID = NV_ENC_PRESET_DEFAULT_GUID; //encoder->GetPresetGUID();
...
EncodeConfig MakeTilerConfiguration(char *inputFilename, char *outputFilenameFormat,
	const unsigned int height, const unsigned int width,
	const size_t tileRows, const size_t tileColumns,
	const unsigned int codec, char *preset, const unsigned int fps, const unsigned int gop_length,
	const size_t bitrate, const unsigned int rcmode, const unsigned int deviceId)
{
...
int ExecuteTiler(const std::string inputFilename, const std::string outputFilenameFormat,
	unsigned int height, unsigned int width,
	size_t tileRows, size_t tileColumns,
	unsigned int codec, std::string preset, unsigned int fps, unsigned int gop_length,
	size_t bitrate, unsigned int rcmode, unsigned int deviceId)
{
	EncodeConfig configuration = MakeTilerConfiguration(
		const_cast<char*>(inputFilename.c_str()), 
		const_cast<char*>(outputFilenameFormat.c_str()),
		height, width,
		tileRows, tileColumns,
		codec, const_cast<char*>(preset.c_str()), fps, gop_length,
