...
/*        Frame* copy_frame(const Frame *frame)
    ...
    class SplitFourColumnsKernel : public BatchedKernel, public VideoKernel {
    public:
        SplitFourColumnsKernel(const KernelConfig& config)
                : BatchedKernel(config)
        { }

        static constexpr size_t rows = 1, columns = 4;
    ...
    REGISTER_KERNEL(SplitFourColumns, SplitFourColumnsKernel)
    .device(DeviceType::CPU)
    .num_devices(Kernel::UnlimitedDevices);


    class SplitFourRowsKernel : public BatchedKernel, public VideoKernel {
    public:
        SplitFourRowsKernel(const KernelConfig& config)
                : BatchedKernel(config)
        { }

        static constexpr size_t rows = 4, columns = 1;
    ...
    REGISTER_KERNEL(SplitFourRows, SplitFourRowsKernel)
    .device(DeviceType::CPU)
    .num_devices(Kernel::UnlimitedDevices);
